...
if (empty(PASSWORD) === false && (isset($_SESSION['pheditor_admin'], $_SESSION['pheditor_password']) === false || $_SESSION['pheditor_admin'] !== true || $_SESSION['pheditor_password'] != PASSWORD)) {
	if (isset($_POST['pheditor_password']) && empty($_POST['pheditor_password']) === false) {
		...
		if ($password_hash === PASSWORD) {
			...
			$error = 'Intenta denuevo la contraseña no es correcta. (Pero si esta facil es admin)';
		...
		die('Session expirada.');
	}

	die('<title>PHP Editor</title><form method="post"><div style="text-align:center"><h1><a href="https://github.com/Dakh4r" target="_blank" title="PHP file editor" style="color:#444;text-decoration:none" tabindex="3">PHP Editor</a></h1>' . (isset($error) ? '<p style="color:#dd0000">' . $error . '</p>' : null) . '<input id="pheditor_password" name="pheditor_password" type="password" value="" placeholder="Contraseña&hellip;" tabindex="1"><br><br><input type="submit" value="Login (Pass: admin)" tabindex="2"></div></form><script type="text/javascript">document.getElementById("pheditor_password").focus();</script>');
}
		...
		case 'save':
			...
			if (isset($_POST['file']) && isset($_POST['data']) && (file_exists($file) === false || is_writable($file))) {
				if (file_exists($file) === false) {
					...
					echo json_success('Archivo Guardado');
				} else if (is_writable($file) === false) {
					echo json_error('Archivo NO escribible');
				} else {
					if (in_array('editfile', $permissions) !== true) {
						die(json_error('Permiso denegado'));
					}
		...
		case 'password':
			...
			if (isset($_POST['password']) && empty($_POST['password']) === false) {
				...
				if (is_writable(__FILE__) === false) {
					die(json_error('Archivo no es leible'));
				}
				...
				echo json_success('Password cambiada');
			}
			break;
			...
			if (isset($_POST['path']) && file_exists(MAIN_DIR . $_POST['path']) && check_path(MAIN_DIR . $_POST['path'])) {
				...
				if ($_POST['path'] == '/') {
					...
					if (count(scandir($path)) !== 2) {
						echo json_error('El directorio esta limpio');
						...
						echo json_success('Directorio borrado sactisfatoriamente');
					}
				} else {
					...
					if (is_writable($path)) {
						...
						echo json_success('Archivo borrado');
					} else {
			...
			if (isset($_POST['path']) && file_exists(MAIN_DIR . $_POST['path']) && isset($_POST['name']) && empty($_POST['name']) === false) {
				...
				if ($_POST['path'] == '/') {
					...
					if (is_writable($path) === false) {
						echo json_error('Nombre no disponible');
					} else {
						rename($path, $new_path);

						echo json_success('Directorio renombrado');
					}
				} else {
					...
					if (is_writable($path)) {
						...
						echo json_success('Archivo renombrado');
					} else {
	...
	<script type="text/javascript">
		...
		$(function() {
			...
			$("a.change-password").click(function() {
				var password = prompt("Ingrese la nueva contraseña:");
			...
			$(".dropdown .new-file").click(function() {
				...
				if (path.length > 0) {
					var name = prompt("Nombre del Archivo:", "new-file.php"),
					...
					alertBox("Warning", "Selecciona el directorio, sino hay. Crealo", "yellow");
				}
			});
			...
			$(".dropdown .new-dir").click(function() {
				...
				if (path.length > 0) {
					var name = prompt("Pls pon el nombre del directorio:", "new-dir"),
						end = path.substring(path.length - 1),
						dir = "";
					...
					alertBox("Warning", "PLS Selecciona el directorio", "yellow");
				}
			});
			...
			$(".dropdown .save").click(function() {
				...
				if (path.length > 0) {
					...
					alertBox("Warning", "PLS Selecciona el Archivo", "yellow");
				}
			});
			...
			$(".dropdown .delete").click(function() {
				...
				if (path.length > 0) {
					...
					alertBox("Warning", "PLS Selecciona el directorio o Archivo", "yellow");
				}
			});
			...
			$(".dropdown .rename").click(function() {
				...
				if (path.length > 0) {
					...
					var name = prompt("Pls ingresa el nombre:", new_file_name);
...
<body>
	...
	<div class="container-fluid">
		...
		<div class="row p-3">
			<div class="col-md-3">
				<h1><a href="https://github.com/Dakh4r" target="_blank" title="Phpeditor">PhP Editor</a></h1>
			</div>
			<div class="col-md-9">
				<div class="float-left">
					<div class="dropdown float-left">
						<button class="btn btn-secondary dropdown-toggle" type="button" id="fileMenu" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">Opciones</button>
						<div class="dropdown-menu" aria-labelledby="fileMenu">
							<?php if (in_array('newfile', $permissions)) { ?>
								<a class="dropdown-item new-file" href="javascript:void(0);">New archivo <span class="float-right text-secondary">N</span></a>
							<?php } ?>

							<?php if (in_array('newdir', $permissions)) { ?>
								<a class="dropdown-item new-dir" href="javascript:void(0);">New Directorio</a>
							<?php } ?>

							<?php if (in_array('uploadfile', $permissions)) { ?>
								<a class="dropdown-item upload-file" href="javascript:void(0);">Upload archivo <span class="float-right text-secondary">U</span></a>
							<?php } ?>
							...
							<?php if (in_array('deletefile', $permissions) || in_array('deletedir', $permissions)) { ?>
								<a class="dropdown-item delete disabled" href="javascript:void(0);">Borrar <span class="float-right text-secondary">D</span></a>
							<?php } ?>
		...
		<div class="row px-3">
			<div class="col-lg-3 col-md-3 col-sm-12 col-12">
				<div id="search">
					...
					<input type="text" value="" class="form-control mb-3 search-input" placeholder="Buscar&hellip;" autocomplete="off">
				</div>
				...
				<div class="col-12">
					<div class="card">
						<div class="card-block">
							<div id="terminal">
								<div>
									<button type="button" class="btn btn-light float-right ml-1 clear" style="display: none;">Vaciar</button>
									<button type="button" class="btn btn-light float-right ml-1 copy" style="display: none;">Copiar al portapapeles</button>
			...
			<div class="modal-dialog">
				<div class="modal-content">
					<div class="modal-header">
						<h4 class="modal-title">Upload Archivo</h4>
	...
	<div class="modal fade" id="helpModal">
		<div class="modal-dialog modal-lg">
			<div class="modal-content">
				<div class="modal-header">
					<h6 class="modal-title">Nada interesante (Probando)</h6>
					<button type="button" class="close" data-dismiss="modal">&times;</button>
				</div>
				<div class="modal-body">
					<div class="row">
